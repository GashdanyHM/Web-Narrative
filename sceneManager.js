...
drawActionSelection: function() {
    ...
    handlePauseMenuClick: function() {
        ...
        if (btns[0] && btns[0].isMouseOver()) { isPaused = false; } // Continue
        if (btns[1] && btns[1].isMouseOver()) { isPaused = false; loadScene('mainMenu'); } // Main Menu
        if (btns[2] && btns[2].isMouseOver()) { isPaused = false; previousGameState = gameState; loadScene('achievements'); } // Achievements
        if (btns[3] && btns[3].text === "Let's call it a day" && btns[3].isMouseOver()) {
            isPaused = false; 
            loadScene('ending_eternal_sleep');
            return; // 增加 return
        }
